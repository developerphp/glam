...
<script>
    ...
    function blockdayadd()
    {
        $.fn.modal({
            ...
            height:     "100px",
            layout:     "elastic",
            url:        "<?php echo base_url() ?>busycms/blockdayadd/",
            padding:    "50px"
            //animation:  "flipInX"
        });
    }
    
    function deleteblockdaydo(id)
    {
        $.ajax({
            url: '<?php echo base_url() ?>busycms/deleteblockday/'+id,
            success: function(data) { $('#sezgin').html(data); }
        });
    }
    
    function deleteblockday(id) {
        $('.notification .hide').click();
        txt='<br/>Delete is blockday <br/><br/><a style="color:#000;" href="javascript:void(0)" onclick="deleteblockdaydo('+id+')">Yes</a> or \n\
...
<!--                        <input type="text" name="search" class="search_user" />-->
                        &nbsp;
                        <button onclick="blockdayadd()">Block Day</button>
                    </div>
                    <div id="productslist">                        
                        <ul>
                        <?php 
                        $aylar=array("","Ocak","Şubat","Mart","Nisan","Mayıs","Haziran","Temmuz","Ağustos","Eylül","Ekim","Kasım","Aralık");
                        $sql=$this->db->query("select * from yasak_gunler order by tarih asc");
                                ...
                                <table>
                                    <tr>
                                        ...
                                        <td width="100" align="right" style="padding-right:20px;">
                                            <a class="icon_tip" href="javascript:void(0);" onclick="deleteblockday(<?php echo $s->id ?>)">X</a>
                                        </td>
                                    </tr>
                                </table>
                            </li>
